...
const SECTIONS = [
  {
    t: '一、我们收集的信息',
    p: '为提供核心服务，我们会收集：① 微信授权登录产生的账号标识（openid）；② 你主动填写的昵称与头像；③ 你主动设置的忌口、辣度、咸淡、技能水平等口味偏好；④ 你产生的问答历史、生成的菜谱与收藏记录；⑤ 为防滥用统计的调用次数。',
  },
  {
    t: '二、信息的使用',
    p: '上述信息用于：向你展示个性化菜谱推荐（口味偏好会注入小伴生成过程）、保存你的问答与收藏以便随时回看、以及控制单日调用以保障服务质量与成本。',
  },
  {
    t: '三、信息的存储与保护',
    p: '你的数据存储于境内服务器（PostgreSQL 数据库），我们采取访问控制与加密传输等合理措施保护数据安全。调用过程可能将你的问题与食材内容发送至大模型服务方用于生成结果，我们会在传输中做必要脱敏。',
  },
  {
    t: '四、信息共享',
